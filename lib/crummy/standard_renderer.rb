module Crummy
  class StandardRenderer
    ...
    include ActionView::Helpers::TagHelper unless included_modules.include?(ActionView::Helpers::TagHelper)
    ...
    def render_crumbs(crumbs, options = {})
      ...
      local_global = lambda { |crumb, global_options, param_name| last_hash.call(crumb).key?(param_name.to_sym) ? last_hash.call(crumb)[param_name.to_sym] : global_options[param_name.to_sym] }

      case options[:format]
      when :html
        crumb_string = crumbs.map do |crumb|
          if local_global.call(crumb, options, :right_side)
            nil
          else
            crumb_to_html(crumb,
        ...
        crumb_string = crumbs.map do |crumb|
          if local_global.call(crumb, options, :right_side)
            ...
            crumb_to_html_list(crumb,
                               ...
                               local_global.call(crumb, options, :last_class),
                               (crumb == crumbs.first),
                               (crumb == crumbs.find_all { |c| !last_hash.call(c).fetch(:right_side, false) }.compact.last),
                               local_global.call(crumb, options, :microdata),
                               local_global.call(crumb, options, :last_crumb_linked),
                               local_global.call(crumb, options, :truncate),
        ...
        crumb_right_string = crumbs.reverse.map do |crumb|
          if !local_global.call(crumb, options, :right_side)
            ...
            crumb_to_html_list(crumb,
                               ...
                               local_global.call(crumb, options, :last_class),
                               (crumb == crumbs.first),
                               (crumb == crumbs.find_all { |c| !local_global.call(c, options, :right_side) }.compact.last),
                               local_global.call(crumb, options, :microdata),
                               local_global.call(crumb, options, :last_crumb_linked),
                               local_global.call(crumb, options, :truncate),
    ...
    def crumb_to_html_list(crumb, links, li_class, first_class, last_class, is_first, is_last, with_microdata, last_crumb_linked, truncate, separator = '')
      name, url, options = crumb
      options = {} unless options.is_a?(Hash)
      can_link = url && links && (!is_last || last_crumb_linked) && !(%r{</a} =~ name)
      html_classes = []
      ...
      html_options = html_classes.empty? ? {} : { class: html_classes.join(' ').strip }
    ...
    def crumb_to_xml(crumb, links, separator, _is_first, _is_last)
      name, url = crumb
